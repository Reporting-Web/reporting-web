...
@Component({
  ...
  styleUrls: ['./rapport-radio.component.css', '.../../../src/assets/css/newStyle.css', 
...
valeurUnderTestRadio =100;
  ...
  GetColumns() {
    this.cols = [
      ...
      { field: 'count', header: this.i18nService.getString('CountRadio') || 'CountRadio', type: 'text', width: '10%' }, // exam Count
  ...
  GetColumnsMedecin() {
    this.colsMedecin = [
      ...
      { field: 'count', header: this.i18nService.getString('CountRadio') || 'CountRadio', type: 'text', width: '30%' }, // exam Count
    ];
  }
  DateTempNew: any;
  ...
  GetData() {
    if (this.dateDeb == null || this.dateFin == null) {
      ...
      this.GetAllDdeExamenRadio();
    }
  }
  theme: string | ThemeOption = 'light';
  groupedBySociete: EChartsCoreOption | null = null;

  chartGroupedBySociete(valeur1: any = 0, valeur2: any = 0, valeur3: any = 0, valeur4: any = 0, totalAdmission: any = 0, valeur5: any = 0): void {  // void return type
    this.groupedBySociete = {
      title: {
        left: '50%',
        text: ' عدد صور الأشعة حسب الجهة ',
  ...
  dataDdeExamenRadio = new Array<any>();
  ...
  GetAllDdeExamenRadio() {
    ...
    this.loadingData = false;
      ...
      this.dataDdeExamenRadio.forEach(group => {
      ...
      this.chartGroupedBySociete(this.countPatientPerCabAndSociete374hayet, this.countPatientPerCabAndSociete375niyebet,
  ...
  calculateTotal(): number {
    return this.dataDdeExamenRadio.reduce((sum, item) => sum + item.countPatient, 0);
  }
  calculateTotalExam(): number {
    return this.dataDdeExamenRadio.reduce((sum, item) => sum + item.count, 0);
  }

  calculateTotalMedecin(): number {
    return this.dataGroupedMedecin.reduce((sum, item) => sum + item.count, 0);
  }
  selectedExamen!: any;
  selectedMedecin!: any;
  ...
  onRowUnselect(event: any) {
    ...
    this.dataGroupedMedecin = new Array<any>();
  }
  ...
  GetChartRoundParFamille() {
    const familyCounts: { [family: string]: number } = {};
    this.dataDdeExamenRadio.forEach(item => {
  ...
  dataGroupedMedecin = new Array<any>();
  ...
  GetAllDdeForMedecin(codePrestation: number) {
    ...
    this.rapportService.GetAllDdeExamenLabByDateAndCodePrestation(this.dateDeb, this.dateFin, codePrestation).subscribe((data: any) => {
      this.dataGroupedMedecin = this.GroupedDataByMedecin(data);
      this.loadingData = false;
    }
    )
  }
  ...
  PostLocalSotrageValeur(){
    localStorage.setItem("underRadio" , this.valeurUnderTestRadio.toString());
    this.CtrlAlertify.PostionLabelNotification();
  ...
  ChartBarCout(data: any,totalCout:any) { 
  ...
  calculateTotalCout(): number {
    return this.dataDdeExamenRadio.reduce((sum, item) => sum + item.coutFactureTotal, 0);
  }
