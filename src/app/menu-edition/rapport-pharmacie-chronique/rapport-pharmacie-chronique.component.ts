...
@Component({
  ...
  styleUrls: ['./rapport-pharmacie-chronique.component.css', '.../../../src/assets/css/newStyle.css', '.../../../src/assets/css/StyleApplication.css'],
...
export class RapportPharmacieChroniqueComponent implements OnInit {
  ...
  dataPharmacieChronique = new Array<any>();
  ...
  calculateTotalCout(): number {
    return (this.dataPharmacieChronique.reduce((sum, item) => sum + item.coutFactureTotal, 0)).toFixed(3);
  }
  ...
  GetAllPrescriChronique(){
    this.IsLoading = true;
    this.rapportService.GetAllPrescritpionByDateAndChronique(this.dateDeb, this.dateFin , true).subscribe((data: any) => {
      this.loadingComponent.IsLoading = false;
      this.IsLoading = false;

      this.dataPharmacieChronique = this.aggregateData(data);
